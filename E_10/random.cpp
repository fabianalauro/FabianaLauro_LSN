...
Random::Random (string primes, string seed, int n) 
{
   int s[4];
   int p1, p2;
   ifstream Primes (primes) ;
   if (Primes.is_open())
   {
      for (int i=0; i<n+1; i++)
      {
         Primes >> p1 >> p2 ;
         if (Primes.eof())
            cerr << "Attention! You have reached the " << primes << " end of file!" << std::endl ;
      }
   }
   else
      cerr << "PROBLEM: Unable to open Primes" << endl;
...
{
    double y = this->Rannyu();
    return -(1./lambda) * log(1 - y) ;
}

 double Random::Lorentz (double x0, double gamma)
{
    double y = this->Rannyu();
