...
void Blocking::AddProg (double n, std::ofstream & Write)
{
    m_val += n ;
    m_counter ++ ;
    ...
    if (m_counter % m_L == 0)
    {
        ...
        Write << j-1 << " " << m_A / (double)(j) << " " << m_sigma << std::endl ;
...
void Blocking::AddFinal (double n, double nblk, std::ofstream  & Write)
{
    m_val += n ;
    m_counter ++ ;
